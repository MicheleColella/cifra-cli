...
func TestScanDiff_DetectsEnvFile(t *testing.T) {
	// A .env file staged → file-level rule triggers even with empty content
...
func TestScanDiff_EntropyDetection(t *testing.T) {
	// Mixed-case alphanumeric value (>4.5 bits entropy) near a credential var name.
...
func TestScanDiff_EntropyIgnoresLowEntropy(t *testing.T) {
	// A low-entropy value near a credential variable name should not trigger.
